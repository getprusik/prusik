...
def test_builtin_registry_has_both_detectors():
    assert set(detectors.BUILTIN) == {"binding", "ci-orphan-specs", "convention-rules", "test-reach"}
...
def test_load_default_is_builtins():
    ...
    assert set(reg) == {"binding", "ci-orphan-specs", "convention-rules", "test-reach"}
...
def test_load_disabled_drops():
    ...
    assert set(reg) == {"binding", "ci-orphan-specs", "convention-rules"}
...
def test_load_skips_malformed_local(capsys=None):
    with tempfile.TemporaryDirectory() as td:
        ...
        reg = detectors.load(root=root, config={})
        assert set(reg) == {"binding", "ci-orphan-specs", "convention-rules", "test-reach"}  # neither bad file registered
