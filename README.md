...
| **[roborev](https://github.com/kenn-io/roborev)** | Continuous code review | Background per-commit review for agents (git-hook triggered) + auto-fix loop; isolated worktrees; multi-harness | If you want review on *every commit* rather than at phase boundaries. Prusik gates review at sprint phases (process-centric); roborev is review-centric + continuous. **Composes** — prusik can consume its non-zero exit via `behavior_regression`. |
| **[Graphify](https://github.com/Graphify-Labs/graphify)** | Codebase knowledge graph | Code/docs → queryable graph (tree-sitter AST ~40 langs, `EXTRACTED` vs `INFERRED` edges, community detection, traversal queries) | If you want deep codebase *understanding* — traverse instead of grep. **Composes** with prusik: feed its graph into scoping/cartography. Prusik's built-in `discovery` is the lighter version. |
